...
提交录屏可直接照 [mini_agent/docs/video-demo-commands.md](mini_agent/docs/video-demo-commands.md) 中的顺序执行，里面包含测试、工具调用、session 恢复、memory 召回和 trace 展示的完整指令。
...
`python -m pytest -m live -q` 跑的是**真实外部 API smoke 测试**，不是 mock。它覆盖 4 条最小真实链路：

- 真实 DeepSeek 直接回复；
- 真实 DeepSeek + `calculator` 工具链路；
- 真实 DeepSeek + 高德 `weather` 工具链路；
- 真实 DeepSeek + Tavily `search` 工具链路。

这个命令用于证明**真实 LLM API、工具调用和 runtime loop 能实际跑通**；它不替代完整测试覆盖，完整功能覆盖主要看普通 `pytest`。
